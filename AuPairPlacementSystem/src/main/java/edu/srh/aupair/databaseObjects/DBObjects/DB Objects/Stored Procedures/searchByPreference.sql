...
SET @WHERECLAUSE =' Where P.IS_ACTIVE=1';
     ...
     #SET @filterCondition=CONCAT(' AND( AU.HOBBIES LIKE ''%',RANDOMSEARCH,'%'' OR ( RF.RATEDBYHOST=1 AND RF.COMMENTS LIKE ''%',RANDOMSEARCH,'%''))');
	 #END IF;
     IF RATINGS <> 0 THEN	
     ...
     SET @filterCondition=CONCAT(@filterCondition,'');
	 SET @WHERECLAUSE =CONCAT(@WHERECLAUSE,@filterCondition);
     SET @AVGRATINGPARAM=concat(@AVGRATINGPARAM,',AVG(RF.RATINGS) RATINGS');
     SET @RATINGSQUERY=CONCAT(@RATINGSQUERY,' LEFT JOIN RATING_FEEDBACK RF ON RF.AU_PAIR_ID=AU.AU_PAIR_ID');
	 END IF;
     #IF (RANDOMSEARCH <> '' OR RATINGS <> 0)THEN
     #SET @RATINGSQUERY=CONCAT(@RATINGSQUERY,' LEFT JOIN RATING_FEEDBACK RF ON RF.AU_PAIR_ID=AU.AU_PAIR_ID');
     #	 END IF;
     ...
     #END IF;
	 set @v_Query=CONCAT('SELECT AU.*,P.*,CC.COUNTRY_NAME,A.*,L.*',@AVGRATINGPARAM,' FROM PERSON P INNER JOIN au_pair AU ON 
     ...
     country_currency CC ON CC.COUNTRY_CURRENCY_ID=A.COUNTRY_CURRENCY_ID ',@RATINGSQUERY);
     
     ELSEIF PERSONTYPE ='AUPAIR'THEN
     
     #IF RANDOMSEARCH <> ''THEN	
     #SET @filterCondition=CONCAT(' AND ( RF.RATEDBYHOST=0 AND RF.COMMENTS LIKE ''%',RANDOMSEARCH,'%'')');
	 #SET @WHERECLAUSE =CONCAT(@WHERECLAUSE,@filterCondition);
	 #END IF;
	 ...
	 SET @WHERECLAUSE =CONCAT(@WHERECLAUSE,@filterCondition);
     SET @AVGRATINGPARAM=CONCAT(@AVGRATINGPARAM,',AVG(RF.RATINGS) RATINGS');
	SET @RATINGSQUERY=CONCAT(@RATINGSQUERY,' LEFT JOIN RATING_FEEDBACK RF ON RF.HOST_ID=H.HOST_ID');
	 END IF;
     
     #IF (RANDOMSEARCH <> '' OR RATINGS <> 0)THEN
     #SET @RATINGSQUERY=CONCAT(@RATINGSQUERY,' LEFT JOIN RATING_FEEDBACK RF ON RF.AU_PAIR_ID=AU.AU_PAIR_ID');
     #	 END IF;
	 set @v_Query=CONCAT('SELECT H.*,P.*,CC.COUNTRY_NAME,A.*,L.*',@AVGRATINGPARAM,' FROM PERSON P INNER JOIN HOSTUSER H ON 
    ...
    ON RF.HOST_ID=H.HOST_ID',@RATINGSQUERY);
	 END IF;
