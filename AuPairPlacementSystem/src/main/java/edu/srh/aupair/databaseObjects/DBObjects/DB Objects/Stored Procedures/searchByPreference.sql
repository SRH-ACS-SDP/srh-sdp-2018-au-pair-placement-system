...
SET @WHERECLAUSE =' Where P.IS_ACTIVE=1';
     ...
     #END IF;
	 set @v_Query='SELECT AU.*,P.*,CC.COUNTRY_NAME,A.*,L.*,AVG(RF.RATINGS) FROM PERSON P INNER JOIN au_pair AU ON 
	 ...
	 SET @WHERECLAUSE =CONCAT(@WHERECLAUSE,@filterCondition);
	 END IF;
	 set @v_Query='SELECT H.*,P.*,CC.COUNTRY_NAME,A.*,L.*,AVG(RF.RATINGS) FROM PERSON P INNER JOIN HOSTUSER H ON 
